...
public class PetService implements BaseService<Pet> {
    ...
    private void validatePet(Pet pet) {
        ...
        if (pet.getTailLength() == null) {
            throw new ValidationException("Длина хвоста котика не может быть пустой");
        }
        if (pet.getTailLength() < 0) {
            throw new ValidationException("Длина хвоста котика не может быть отрицательной");
        }
    }
} 