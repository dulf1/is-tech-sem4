...
import ru.dulfi.petservice.repository.PetRepository;
import ru.dulfi.petservice.exception.ResourceNotFoundException;
import ru.dulfi.petservice.exception.ValidationException;
...
public class PetService implements BaseService<Pet> {
    ...
    public Page<Pet> findPetsByName(String name, Pageable pageable) {
        System.out.println("Ищем котиков с именем, содержащим: " + name);
        Page<Pet> page = petRepository.findByNameContainingIgnoreCase(name, pageable);
        page.getContent().forEach(pet -> System.out.println("Найден котик " + pet.getName() + " с длиной хвоста: " + pet.getTailLength()));
        return page;
    }
    ...
    private void validatePet(Pet pet) {
        ...
        if (pet.getOwnerId() == null) {
            throw new ValidationException("У котика должен быть хозяин (ownerId)");
        }
